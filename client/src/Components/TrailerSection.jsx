...
const TrailerSection = () => {
  const [currenctTrailer, setCurrentTrailer] = useState(dummyTrailers[0]);
  return (
      ...
      <div className="group grid grid-cols-4 gap-4 md:gap-8 mt-8 max-w-3xl mx-auto">
        {dummyTrailers.map((trailer) => (
          ...
          >
            ...
            <PlayCircleIcon
              strokeWidth={1.6}
              className="absolute top-1/2 left-1/2 w-5 md:w-8 h-5 md:h-12 -translate-x-1/2 -translate-y-1/2 transform"
            />
          </div>
        ))}
      </div>
